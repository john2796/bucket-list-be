...
router.get("/user/friends", (req, res) => {
  friendsDb
    ...
    .then(response => {
      if (response.length > 0) res.status(200).json({ friends: response })
      else res.status(404).json({ message: "No friends found" })
    })
    .catch(error => {
...
router.post("/user/friends/:friend_id", (req, res) => {
  if (req.user.id == req.params.friend_id) {
    res
      .status(403)
      .json({ message: "Can not be friends with yourself.  Go find some." })
    return
  }
...
router.delete("/user/friends/:friend_id", (req, res) => {
  friendsDb
    ...
    .then(response => {
      ...
      else res.status(200).json({ message: "Unfriended" })
    })
    .catch(error => {
      res.status(500).json({ error: error.message })
    })
